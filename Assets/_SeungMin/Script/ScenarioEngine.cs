...
public class ScenarioEngine : MonoBehaviour
{
    ...
    public IEnumerator PlayScript(string script, GameObject npc)
    {
        ...
        foreach (string token in script.Split('\n'))
        {
            ...
            else if (fun == "restrictPlayer")
            {
                isPlayerRestricted = true;
            }
            else if (fun == "releasePlayer")
            {
                isPlayerRestricted = false;
            }
            else if (fun == "rotateCamera")
            {
                int cameraIndex = int.Parse(tokens[1]);
    ...
    IEnumerator RotateCameraTowards(GameObject camera, Vector3 targetPosition)
    {
        ...
        {
            camera.transform.rotation = Quaternion.Slerp(camera.transform.rotation, targetRotation, Time.deltaTime * 2);
            yield return null;
        }
    ...
    void Update()
    {
        if (isPlayerRestricted)
        {
            ...
            player.GetComponent<ThirdPersonController>().RestrictMovement(true);
        }
        else
        {
            GameObject player = map["Player"];
            player.GetComponent<ThirdPersonController>().RestrictMovement(false);
        }
    }
}
